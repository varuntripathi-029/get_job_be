...
- **Two-model LLM strategy.** A cheap model (`CLASSIFIER_MODEL`) gates relevance; only survivors reach the expensive model (`EXTRACTOR_MODEL`) for structured extraction. A rule-based pre-filter runs before either, at zero cost.
